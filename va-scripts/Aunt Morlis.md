# Aunt Morlis — Voice Actor Script
...
*mor18 is a merged recording: the original script concatenated two message IDs with the player's name spoken in between (a silent-tail engine bug), so this single take covers the whole line, name included, as "Chosen One." mor19 was cut in a later release on purpose — not a gap to fix.*
...
*End of script — 26 lines total (mor1–mor27, mor19 intentionally not in use). Tag numbers follow ascending message-ID order, not this document's topical grouping.*
